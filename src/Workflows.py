...
def record_and_create_pipeline(pipeline: ModuleType = spectral):
...
def create_pipeline_for_subject(subject, pipeline: ModuleType = spectral, choose=False):
    """
...
def find_best_hyperparams_for_subject(subject=None, pipeline: ModuleType = spectral, choose=False):
    epochs, labels = load_epochs_for_subject(subject, choose)
    best_hyperparams = bayesian_opt(epochs, labels, pipeline)
    save_hyperparams(best_hyperparams, pipeline.name, subject)


def find_best_pipeline_for_subject(subject=None, pipeline: ModuleType=csp):
...
def record_with_live_retraining(subject, pipeline: ModuleType = spectral, choose=False):
...
if __name__ == "__main__":
    load_epochs_for_subject("Synthetic")
